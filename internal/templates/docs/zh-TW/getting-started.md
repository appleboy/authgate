...
3. **`client_secret`** — 只有 _機密_ 客戶端才會拿到（伺服器端網頁應用、Client Credentials 服務）。公開客戶端（SPA、行動、CLI）沒有 secret。
...
7. **Resource 識別字** — _除非您在整合 MCP server 或多 RS 部署且需要 audience binding（[RFC 8707](https://datatracker.ietf.org/doc/html/rfc8707)），否則略過此項_。要帶入 `resource=` 的絕對 http(s) URI，這樣簽發的 access token 的 `aud` 才會等於您的 resource server。
...
> **非 OIDC / MCP 客戶端**：AuthGate 也在 `/.well-known/oauth-authorization-server` 發布一份 [RFC 8414](https://datatracker.ietf.org/doc/html/rfc8414) 授權伺服器 metadata，會宣告 `resource_indicators_supported: true` 與 `device_authorization_endpoint`，是 MCP 與純 OAuth 2.1 客戶端慣用的版本。兩份文件重疊欄位內容相同；依您函式庫的需求挑一份即可。
...
在 token 請求帶 `resource=<URL>` 時，access token 的 `aud` claim 會綁到該 resource（RFC 8707）。Resource server 應對自己的識別字驗 `aud` — 見 [JWT 驗證 §Audience Binding](./jwt-verification#audience-binding-rfc-8707)。
