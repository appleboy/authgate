...
| ------------------------------------- | ----------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| 它在描述 _誰_？                       | 終端使用者（身分）                        | 呼叫 API 的授權                                                                                                             |
| 它 _給誰用_？                         | **您的客戶端應用**（`aud=client_id`）     | 它綁定的 resource server（`aud`=每次請求的 `resource` 或 `JWT_AUDIENCE`）                                                   |
| 能當 `Authorization: Bearer` 送 API？ | **不行** — 永遠不行                       | 可以                                                                                                                        |
| 需要驗 `aud`？                        | **要 — 必須等於您的 `client_id`**         | **要 — 必須等於 resource server 的識別字**（見 [JWT 驗證 §Audience Binding](./jwt-verification#audience-binding-rfc-8707)） |
...
8. **`at_hash`** _（選用、建議）_ — 驗證與同時拿到的 access token 相符。
...
> AuthGate 也在 `/.well-known/oauth-authorization-server` 發布平行的 [RFC 8414](https://datatracker.ietf.org/doc/html/rfc8414) 文件，提供給非 OIDC 的 OAuth 2.1 / MCP 客戶端。兩份文件共享的欄位內容保持一致 — 依您函式庫的需求挑一份用即可。
...
- **沒驗簽就解析 ID token**。千萬別這樣做 — 沒驗簽前的 JWT 是 _未認證_ 的。
- **對 _access_ token 驗 `aud=client_id`**。Access token 的 `aud` 是 resource server 的識別字（RFC 8707 `resource` 或 `JWT_AUDIENCE`），不是您的 `client_id`。`aud=client_id` 的驗法只適用於 ID token。
