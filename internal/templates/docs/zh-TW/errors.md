...
| `invalid_target`            | 一個或多個 `resource=` 參數未通過 RFC 8707 驗證（非 http(s)、含 fragment、空 host、超過 10 個、超過 1024 字元） | 修正請求 — 見下方 [Resource Indicator 錯誤](#resource-indicator-錯誤-rfc-8707) |
...
| `invalid_target`         | 400  | `resource=` 格式不對，**或**（`refresh_token` / `authorization_code` / `device_code` 文法下）要求的 resource 不在原授權子集（RFC 8707 §2.2 narrowing rule） | 修正請求 — 見下方 [Resource Indicator 錯誤](#resource-indicator-錯誤-rfc-8707)。同一值不要重試                        |
...
任何 `resource=` 參數驗證失敗都會回 `invalid_target`。分兩類：
...
**Subset rule**（RFC 8707 §2.2 — token endpoint 上，對曾經做過 resource binding 的 grant）：
...
| `client_credentials`                           | 任何 `resource=` 都可接受，只要通過格式驗證                              |

對 `device_code`，遇到 `invalid_target` 時 device code **不會被消耗** — CLI 可修正 resource list 後重試。但 `authorization_code` 依 RFC 6749 是一次性的，遇到錯誤使用者必須從頭走流程。

各 flow 範例見：[Authorization Code Flow](./auth-code-flow)、[Device Authorization Flow](./device-flow)、[Client Credentials Flow](./client-credentials)。
